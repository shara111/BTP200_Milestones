...
namespace sdds
{
	...
	std::ostream& Book::write(std::ostream& os) const
	{
		...
		{
			os << ' ';
			if (m_authorName and strlen(m_authorName) > SDDS_AUTHOR_WIDTH)  //Print the box that contains only the max Title width
			{
	...
	std::istream& Book::read(std::istream& is)
	{
		...
		if (conIO(is))
		{
			...
			is.ignore(3000, '\n');		//Ignore 3000 instead of no arguments
		...
		else
		{
			...
			strcpy(m_authorName, authorNameInput);// Not using strncpy, cuz we have already allocated the memory
